# Mysterium · 古典马赛塔罗 Demo

一个无需安装依赖、可直接运行的多牌阵塔罗网页原型。

在线体验：[https://old-paper.github.io/mysterium-tarot/](https://old-paper.github.io/mysterium-tarot/)

## 运行

最简单的方法是直接双击 `index.html`。也可以在此目录启动本地静态服务器：

```powershell
python -m http.server 4173
```

然后访问 `http://localhost:4173`。

## 更新网站

本站由 GitHub Pages 从 `main` 分支根目录自动发布。修改完成后，在本目录执行：

```powershell
git add .
git commit -m "描述这次修改"
git push
```

推送完成后，GitHub 会自动重新部署网站。通常稍等片刻即可在上面的在线地址看到更新。

## 当前范围

- 完整 78 张马赛体系组合牌库：22 张 Jean Dodal 主牌与 56 张 BnF 馆藏 Conver 系小牌；明确说明两部分并非同一副历史牌
- 小牌保留钱币、圣杯、宝剑、权杖四组各 14 张的传统点数牌结构
- 单张、三张、五张十字、七张马蹄、二选一、现代凯尔特十字、Waite 1911 凯尔特十字原法、十二宫与四十二张综合牌阵
- 独立的 Waite 1911 原法模式：先固定明确问题，选择并正面放置代表牌，预先确认其面向；从剩余 77 张连续洗牌并切牌三次，再按“覆盖、横跨、冠顶、脚下、身后、面前、自己、居所、希望或恐惧、将发生之事”布置十张牌
- 原法模式支持 Waite 第 7 节记载的可选复占：若第 10 张含义模糊或是宫廷牌，可将它设为新的代表牌，重新确认面向后再洗切三次
- 十二宫可选第 13 张主题牌
- 可实际抽取的 42 张 Waite 全景法：六叠七张、重叠为七叠六张、分组重洗并排成六行七张
- 默认默念问题模式，以及可切换的文字输入模式
- 浏览器加密随机数 + 拒绝采样 + Fisher–Yates 洗牌 + 随机切牌
- 抽牌前后张数与唯一性校验
- 可选正逆位；方向在完整牌库洗牌前独立以 50% 概率决定，这是现代约定，不保证每次一半逆位
- 42 张法可选魔术师或女教皇人物牌，并按原法处理抽离与补位
- 提问教程随牌阵变化，两种输入模式均可见；单张重焦点、二选一先定义 A/B、十二宫和 42 张允许全景意向
- 从抽牌开始固定问题、牌阵、牌位、方向设置和解读详略；完整抽牌记录深冻结，页面与导出使用同一份记录
- 按所选牌阵动态生成牌位、逐张翻牌、基础牌义和牌位导读；关系提纲覆盖全部位置，不冒充已理解问题的个性化 AI 分析
- 复制释读，以及包含问题、牌阵、全部牌位与解读边界的 AI 提示词
- 42 张 AI 提示词默认逐牌详读，也可在抽牌前选择六行概览；所有引用使用唯一的全局牌号 1—42
- 响应式与键盘可访问界面

## 史料边界

- 早期塔罗是 15 世纪欧洲的纸牌游戏，并非有证据支持的古埃及秘典。
- 1783–1784 年 Etteilla 的著作属于可核查的早期塔罗占卜系统文献。
- 除明确列出文献来源的牌法外，现代牌阵没有唯一标准版本；本站在抽牌前固定并展示每个牌位的定义。
- 凯尔特十字十个主体位置与 42 张全景法参考 A. E. Waite 1911 年公开文本。本站保留原有的现代简化版，同时将第 7 节的可复现步骤单独实现为“Waite 1911 凯尔特十字原法”。原法模式必须另选代表牌，故十张结果从剩余 77 张中取得；电子洗切以三轮独立的 Fisher–Yates 洗牌和随机切牌对应原文的三次洗切。
- Waite 第 7 节没有说明用何种操作制造逆位，本站不会把现代的独立 50% 逆位规则冒充原法，因此该模式固定正位。这里的“原法”只指第 7 节牌阵步骤，不表示本站的马赛组合牌面或中文现代整理牌义就是 Waite–Smith 牌面及 Waite 原叙述。
- “左手切牌”写在 Waite 第 8 节的 42 张法，而不是第 7 节凯尔特法；本站没有把它错误加入此模式。
- 42 张法使用完整 78 张牌与 42 张不重复牌；本站实现六行七张、人物牌抽离与随机补位步骤。原法使用人物牌；本站允许自主选择或省略，不按原文的性别规则自动指定。启用时人物牌置于阵外，其余 77 张是有效可入阵牌库。
- 牌义是基于图像与当代反思用途的中文整理，不冒充 Etteilla 原文直译。
- 程序可提高随机公平性、方法一致性与记录可核验性，但不宣称塔罗具有可保证的预测准确率。

## 验证

无需安装依赖的算法与记录回归测试（Node.js）：

```powershell
node tests/methods.test.cjs
```

覆盖完整牌库、拒绝采样边界、小牌组 Fisher–Yates 全排列、42 张分组与人物牌补位、各牌阵张数与方向、固定记录、提示词模式与重置。断言失败会以非零状态退出。随机样本看似均匀不能单独证明公平性，因此测试包含可穷举的小规模算法验证。

浏览器回归测试需 Playwright 与 Chromium / Edge（默认使用 Windows 上的 Edge，可用 `TAROT_BROWSER` 指定浏览器路径）：

```powershell
node tests/browser.test.cjs
```

默认访问 `http://127.0.0.1:4173/`；没有本地服务时测试会临时托管当前目录，结束后自动关闭。可用 `TAROT_TEST_URL` 指定其他测试地址。核心随机算法版本保持 `MYSTERIUM-RNG-2`；固定阅读记录版本为 `MYSTERIUM-READING-1`。这些版本用于核对实现，不是预测准确性或第三方审计认证。

## 牌面与资料来源

- [OpenGameArt · Jean Dodal Major Arcana](https://opengameart.org/content/tarot-cards-major-arcana) — CC0，约 1701 年的马赛体系图像整理
- [BnF / Gallica · 完整历史马赛牌组](https://gallica.bnf.fr/ark:/12148/btv1b10539497f) — 本站 56 张小牌的扫描来源
- [法国国家图书馆 · Jean Dodal, Lyon, 1701](https://catalogue.bnf.fr/ark:/12148/cb391713667)
- [The Metropolitan Museum of Art · Before Fortune-Telling](https://www.metmuseum.org/perspectives/tarot-2)
- [Bibliothèque nationale de France · Etteilla, 1783–1784](https://catalogue.bnf.fr/ark:/12148/cb44214742d)
- [A. E. Waite · The Pictorial Key to the Tarot, Part III](https://en.wikisource.org/wiki/The_Pictorial_Key_to_the_Tarot/Part_3)
- [Astrological System for Reading Tarot · Twelve Houses](https://www.daneel.franken.de/tarot/ATA/courses/astrological/ast.pdf)
- [W3C · Web Cryptography API](https://www.w3.org/TR/webcrypto/#Crypto-method-getRandomValues)

## 下一阶段建议

1. 新增 Etteilla 历史模式，与现代马赛阅读模式分开呈现。
2. 增加抽牌记录、本地日记与多套牌面。
3. 增加完整牌库浏览、搜索与学习模式。
